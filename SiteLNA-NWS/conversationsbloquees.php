...
$req1 = $bdd->prepare('SELECT id_membre_2, timestamp_conversation, l FROM conversations WHERE id_membre_1 = ? AND situation_conversation = ? AND id_membre_situation = ?
ORDER BY timestamp_conversation DESC LIMIT ' .$limit.', 10');
$req1->execute(array($_SESSION['id'], 2, $_SESSION['id']));
$donnees1 = $req1->fetch();
$n = 1;
...
$req3 = $bdd->prepare('SELECT COUNT(id_conversation) as nbr_conversation FROM conversations WHERE id_membre_1 = ? AND situation_conversation = ? AND id_membre_situation = ?');
$req3->execute(array($_SESSION['id'], 2, $_SESSION['id']));
$donnees3 = $req3->fetch();
...
<html>
	<head>
		...
		<title>Conversations bloquées - Les Nouveaux Arrivants</title>
	</head>
	<body>
		<?php include("headerc.php"); ?>
		<section class="sectionconvb">
			<div class="pconb">
				<div class="div1convb">
				...
				<div class="div2convb">
					<div class="div9cadre div10cadre">
						<h2>Vos conversations bloquées</h2>
						<h3><a href="conversations.php" class="aconvb">Voir les conversations amicales</a></h3>
						<h3><a href="conversationslieux.php" class="aconvb">Voir les conversations avec des lieux</a></h3>
					</div>
							...
							<?php 
								if($conversations == 1)
								{
									...
									{	
										...
										<input type='submit' name='debloquer' value='Débloquer' class='boutonnom'/>
										</form></div>";
									}
								}else
								{
						...
						<p>
							<div class="div9cadre div10cadre">
								...
								<?php
									...
									{
										echo "<input type='submit' name='page' value=" .$i. " class='boutonconvb'/>";
									}
								?>
								</form>
							</div>
