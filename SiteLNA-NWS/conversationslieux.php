...
$req1 = $bdd->prepare('SELECT id_membre_2, timestamp_conversation FROM conversations WHERE id_membre_1 = ? AND situation_conversation = ?
 AND l = ? ORDER BY timestamp_conversation DESC');
$req1->execute(array($_SESSION['id'], 1, 1));
$donnees1 = $req1->fetch();
$n = 1;
...
{ 
	do
	{
		$req2 = $bdd->prepare('SELECT nom_l FROM inscrits WHERE id = ?');
...
$req3 = $bdd->prepare('SELECT COUNT(id_conversation) as nbr_conversation FROM conversations WHERE id_membre_1 = ? AND situation_conversation = ? AND l = ? LIMIT '.$limit.', 10');
$req3->execute(array($_SESSION['id'], 1, 1));
$donnees3 = $req3->fetch();
			...
			<div class="pconv">
				...
				<div class="div2conv">
					<div class="div9cadre div10cadre">
						<h2>Vos conversations avec des lieux</h2>
						<h3><a href="conversations.php" class="aconv">Voir les conversations amicales</a></h3>
						...
						<p>
							<div class='div9cadre div10cadre'>
								<form method="post" action="conversationslieux.php" class="form">
								<?php
